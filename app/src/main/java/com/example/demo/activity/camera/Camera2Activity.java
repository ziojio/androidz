...
public class Camera2Activity extends BaseActivity implements SurfaceHolder.Callback, ImageReader.OnImageAvailableListener, TextureView.SurfaceTextureListener {
    ...
    private void showCameraInfo() {
        try {
            ...
            for (String id : ids) {
                ...
                Object lensFacing = characteristics.get(CameraCharacteristics.LENS_FACING);
                Timber.d("CameraCharacteristics.LENS_FACING " + lensFacing);
                if (Objects.equals(lensFacing, CameraCharacteristics.LENS_FACING_FRONT)) {
                    FACING_FRONT = id;
                } else if (Objects.equals(lensFacing, CameraCharacteristics.LENS_FACING_BACK)) {
                    FACING_BACK = id;
                }
                Object supportedHardwareLevel = characteristics.get(CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL);
                Timber.d("CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL " + supportedHardwareLevel);

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    Object version = characteristics.get(CameraCharacteristics.INFO_VERSION);
                    Timber.d("CameraCharacteristics.INFO_VERSION " + version);
                }
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    Object range = characteristics.get(CameraCharacteristics.CONTROL_ZOOM_RATIO_RANGE);
                    Timber.d("CameraCharacteristics.CONTROL_ZOOM_RATIO_RANGE " + range);
                }
            }
            Timber.d("-------------------------------------------------");
        } catch (Exception e) {
        ...
        try {
            mCameraManager.openCamera(mFacing, new CameraDevice.StateCallback() {
                ...
                public void onError(@NonNull CameraDevice camera, int error) {
                    closeCamera();
                    String s = switch (error) {
                        case ERROR_CAMERA_DEVICE -> "Fatal (device)";
                        case ERROR_CAMERA_DISABLED -> "Device policy";
                        case ERROR_CAMERA_IN_USE -> "Camera in use";
                        case ERROR_CAMERA_SERVICE -> "Fatal (service)";
                        case ERROR_MAX_CAMERAS_IN_USE -> "Maximum cameras in use";
                        default -> "Unknown";
                    };
    ...
    public void surfaceChanged(@NonNull SurfaceHolder holder, int format, int width, int height) {
        Timber.d("surfaceChanged width= " + width + ", height=" + height);
    }
